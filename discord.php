...
class DiscordPlugin extends Plugin {
    ...
    function onTicketCreated(Ticket $ticket) {
        ...
        $heading = sprintf('%s CONTROLSTART%sscp/tickets.php?id=%d|#%sCONTROLEND %s'
                ...
                , $ticket->getNumber()
                , __("created"));
        $this->sendToDiscord($ticket, $heading, $plaintext);
    }
    ...
    function onTicketUpdated(ThreadEntry $entry) {
        ...
        $heading = sprintf('%s CONTROLSTART%sscp/tickets.php?id=%d|#%sCONTROLEND %s'
                ...
                , $ticket->getNumber()
                , __("updated"));
        $this->sendToDiscord($ticket, $heading, $plaintext, 'warning');
    }
    ...
    function sendToDiscord(Ticket $ticket, $heading, $body, $colour = 'good') {
        ...
        // Add our custom var
        $custom_vars       = [
            'discord_safe_message' => $this->format_text($body),
        ];
        ...
        try {
            ...
            $ost->logError('Discord posting issue!', $e->getMessage(), true);
