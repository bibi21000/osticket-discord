...
// Validate config
        ...
        if (!$this->cfg instanceof OsticketConfig) {
            error_log("Somehow the configuration wasn't loaded while the Discord notification plugin was attempting to process a ticket.");
            $error = true;
        }
        ...
        if (!$plugin_config->get('discord-webhook-url')) {
            $this->ost->logError('Discord Plugin not yet configured', 'You need to read the Readme and configure a webhook URL before using this.');
            $error = true;
        }

        if (!extension_loaded('curl')) {
            $this->ost->logError('Discord Plugin: curl extension missing', 'You need to install and enable the php_curl extension before we can use it to send notifications.');
            $error = true;
        }

        if (!strlen($plugin_config->get('message-template'))) {
            $this->ost->logInfo('Discord: Regenerating message template', 'It was missing/empty in the plugin config, reverting to default.');
            $this->getConfig()->set('message-template', DiscordPluginConfig::$template);
        }
...
// bail before attempting to send things
            error_log("Discord plugin not ready, No notifications will be attempted until setup is completed.");
            return;
        }
...
// Format the messages we'll send.
            $heading = sprintf('%s CONTROLSTART%sscp/tickets.php?id=%d|#%sCONTROLEND %s'
                    ...
                    , $ticket->getNumber()
                    , __("created"));
            $this->ost->logDebug("Discord Sending...", $heading . "\n" . $plaintext);
            $this->sendToDiscord($ticket, $heading, $plaintext, $colour);
        } catch (Exception $e) {
            $this->ost->logError("Discord: Exception encountered!", sprintf("New ticket error for ticket_id: %s\n%s", $ticket->getId(), $e->getMessage));
            $this->ost->logDebug("Discord Trace", $e->getTraceAsString());
        }
    }
    ...
    private function getColour($entry, $config) {
        // Setup what types of messages we'll pass through to discord, defaults to just user-messages:
...
// See if the admin has allowed Agent responses to be posted to Discord:
...
// Fetch the colour for the post
        ...
        if ($colour === FALSE) {
// Response was not selected for discord notification by admin... 
            $this->ost->logDebug("Discord Ignoring message", "You can use the plugin config to enable this type of notice if desired.");
            return;
        }
...
// Admin created ticket's won't work here.
            $this->ost->logDebug("Discord ignoring message", "Because there is no associated ticket.");
            return;
        }
...
// Check to make sure this entry isn't the first (ie: a New ticket)
        ...
        if ($entry->getId() == $first_entry->getId()) {
            $this->ost->logDebug("Discord ignoring message", "Because we don't want to notify twice on new Tickets");
            return;
        }
...
// Format the messages we'll send
            ...
            $this->ost->logError("Discord: Exception encountered!", sprintf("Reply to ticket error for ticket_id: %s\n%s", $ticket->getId(), $e->getMessage), false);
            $this->ost->logDebug("Discord Trace", $e->getTraceAsString());
        }
    }
     ...
     * @throws \Exception
     */
    function sendToDiscord(Ticket $ticket, $heading, $body, $colour = 'good') {
...
// Filter on subject, and validate regex:
        if ($regex_subject_ignore && preg_match("/$regex_subject_ignore/i", $ticket->getSubject())) {
            $this->ost->logDebug('Discord Ignored Message', 'Discord notification was not sent because the subject (' . $ticket->getSubject() . ') matched regex (' . htmlspecialchars($regex_subject_ignore) . ').');
            return;
        }

        $formatted_heading = $this->discord_text_formatter($heading);
        if (!$formatted_heading) {
            $formatted_heading = '[heading text missing]';
        }

// Pull template from config, and use that. 
        $template          = $this->getConfig()->get('message-template') ?: DiscordPluginConfig::$template;
// Add our custom variables to an array 
        $custom_vars       = [
            'discord_safe_message' => $this->discord_text_formatter($body),
        ];
...
//  'author_link' => $cfg->getUrl() . 'scp/users.php?id=' . $ticket->getOwnerId(), // not included, as these assume Author is a Discord user.
// 'author_icon' => $this->get_gravatar($ticket->getEmail()),
            ...
            'ts'          => time(),
            'footer'      => 'via osTicket Discord Plugin',
            'footer_icon' => 'https://platform.discord-edge.com/img/default_application_icon.png',
...
// After the formatting into sendable format, we set some simpler vars for later use
    ...
    function curl_me($url, $data_string, $payload) {
        ...
        if (!$data_string) {
            $this->ost->logError("Discord: No Url for Curl!", "We need that webhook.. this is an error, because it shouldn't get here without one!");
            return false;
        }
...
// Check the statusCode against common discord posting errors

            switch ($http_response) {
                case 200: {
// this is the good result!
                        $this->ost->logDebug("Discord Notification Success", "Notification to discord successful!");
                        break;
                    }
                case 400: {
                        if ($payload['original']) {
//Simply and resend, something went wonky
                            $this->ost->logWarn("Discord Notification Issue", "There was a problem sending to their server, not sure what yet, but it could be related to the message we sent, so we're simplifying the notice and trying it again resending, you don't have to do anything, unless you get a bunch of these.. then try turning off some config options, or let the dev know.");
                            $payload['original'] = false;
                            $this->simplify_and_resend($url, $payload);
                        }
                        else {
                            $this->ost->logError("Discord Notification Error", sprintf("We tried to resend a message %s to %s however it failed twice, we simplified it and it still failed, so something is wrong.", $payload['message'], $url), true);
                        }
                        break;
                    }
                case 403:
...
// Permissions error with endpoint, unable to post!
                        $this->ost->logError("Discord Notification Error", sprintf("Channel Error attempting to send to: %s\nHttp code: %d\ncurl-error: %d \nBased on http status code, it could be %s.", $url, $http_response, $error_number, $message), true);
...
//TODO: Build a queue to store these in.. even just a folder with json dumps per ticket id.. and what for replies? hmm. 
                        $this->ost->logError("Discord Notification Error", sprintf("Discord Server Error attempting to send to: %s\nHttp code: %d\ncurl-error: %d \nBased on http status code, it's like at their end, working on a way of caching these to resend later.", $url, $http_response, $error_number), true);
                        return;
                    }
            }
        }
        // If we hit here, all was fine.
        $this->ost->logDebug("Discord: Message sent", "Check the thread for a notice, or turn off debugging because it's working fine.");
    }

    /**
     * Formats text according to the Discord
     * formatting rules:https://api.discord.com/docs/message-formatting
     * 
     * @param string $text
     * @return string
     */
    function discord_text_formatter($text) {
        $formatter      = [
            '<' => '&lt;',
            '>' => '&gt;',
            '&' => '&amp;'
